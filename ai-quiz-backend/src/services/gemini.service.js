...
async function generateQuizFromText(knowledgeText, { numQuestions, difficulty, studentClass }) {
    ...
    try {
        ...
        const quizJson = JSON.parse(cleanJsonString(response.text()));
        return quizJson;
    } catch (error) {
...
async function generateAnswerExplanations(questions) {
    ...
    try {
        const result = await model.generateContent(prompt);
        const response = result.response;
        const explanationJson = JSON.parse(cleanJsonString(response.text()));
...
/**
 * Analyzes test results using the Gemini API.
 */
async function analyzeResultsWithAI(questions, studentAnswers) {
    ...
    try {
        const result = await model.generateContent(prompt);
        const response = result.response;
        const analysisJson = JSON.parse(cleanJsonString(response.text()));
        return analysisJson;
    } catch (error) {
